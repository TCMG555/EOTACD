...
public class Chapter1_2 : screen
{
    Game1 game;
    Player player1;
    Player player2;
    ...
    Texture2D backgroundTextureL;
    Texture2D backgroundTextureR;
    Texture2D bgLab;
    ...
    public Chapter1_2(Game1 game, EventHandler theScreenEvent) : base(theScreenEvent)
    {
        this.game = game;
        ...
        bgLab = game.Content.Load<Texture2D>("Chapter1_02FullExample");


        enemy = new Enemy(new Vector2(500, 760), 100f); // สร้างศัตรูที่ตำแหน่งนี้
        enemy.LoadContent(game.Content, "WormWalkLeftt", "WormWalkRight", "WormAttackLeft", "WormAttackRight"); // โหลดเนื้อหาศัตรู

        ladderLRectangle = new Rectangle(250, 350, 10, 420);
    ...
    public void CheckCollision(Player player)
    {
        ...
        Rectangle playerRectangle = new Rectangle((int)player.Position.X, (int)player.Position.Y, 10, 160); // ขนาดของผู้เล่น
    ...
    public override void Update(GameTime gameTime)
    {
        ...
        if (player1.Position.X > 1600 || player2.Position.X > 1600 && Keyboard.GetState().IsKeyDown(Keys.Space))
        {
            ...
            ScreenEvent.Invoke(game.mGameplayCT2_1, new EventArgs());
            return;
        }
        ...
        if (enemySpawned)
        {
            ...
            {
                enemy.Update(gameTime, player1);
                enemy.Update(gameTime, player2);// ตรวจสอบว่าศัตรูมีอยู่แล้วหรือไม่
            }
        }
